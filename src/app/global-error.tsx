...
export default function GlobalError({
    ...
    useEffect(() => {
        ...
        console.error('Fatal Global Exception Encountered:', error);
    ...
    return (
        <html lang="en">
            <body className="bg-slate-50 font-sans antialiased text-slate-900">
                <div className="min-h-screen flex flex-col items-center justify-center p-8 animate-in fade-in zoom-in-95 duration-500">
                    <div className="max-w-4xl w-full">
                        ...
                        <div className="flex items-center gap-5 mb-8 text-rose-600">
                            ...
                            <div>
                                <h1 className="text-3xl font-black tracking-tight text-slate-900">Global Application Crash</h1>
                                <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mt-1">Fatal Layout Runtime Exception</p>
                            </div>
                        </div>

                        {/* Error Container */}
                        <div className="rounded-[32px] border border-rose-200/50 shadow-2xl shadow-rose-900/5 bg-white overflow-hidden">
                            <div className="bg-rose-50/50 border-b border-rose-100/60 p-6 flex flex-col gap-2">
                                ...
                                <p className="font-mono text-sm text-rose-900 font-bold bg-white p-5 rounded-2xl border border-rose-100 break-all shadow-sm">
                                    {error.message || 'Unknown fatal render error occurred during layout execution cycle.'}
                                </p>
                            </div>
                            ...
                            <div className="p-8 space-y-8">
                                ...
                                <div className="pt-4 flex items-center gap-4 border-t border-slate-100 mt-6">
                                    <button
                                        onClick={() => reset()}
                                        className="flex shrink-0 items-center justify-center bg-slate-900 hover:bg-black text-white px-8 h-12 rounded-xl font-bold uppercase tracking-widest shadow-xl shadow-slate-200 transition-all hover:-translate-y-0.5"
                                    >
                                        <RefreshCcw className="h-4 w-4 mr-2" /> Attempt State Recovery
                                    </button>
                                    <button
                                        onClick={() => window.location.href = '/'}
                                        className="flex shrink-0 items-center justify-center px-8 h-12 rounded-xl font-bold uppercase tracking-widest text-slate-500 hover:text-slate-900 border border-slate-200 hover:bg-slate-50 transition-all"
                                    >
